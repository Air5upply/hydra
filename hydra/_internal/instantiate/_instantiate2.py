...
def _normalize_target_whitelist(
    ...
    try:
        return tuple(
            _validate_target_whitelist_pattern(pattern)
...
class _TargetWhitelistPolicy:
    ...
    def resolve(self, inherited: NormalizedTargetWhitelist) -> NormalizedTargetWhitelist:
        if self.reset:
...
def target_whitelist(
    target_whitelist: TargetWhitelist, reset: bool = False
) -> Any:
    """
...
def _warn_legacy_target_whitelist(target: str) -> None:
    stacklevel = 5
    for index, frame in enumerate(inspect.stack(), start=0):
...
def _resolve_target(
    ...
    if isinstance(target, str) or callable(target):
        ...
        elif not _is_target_whitelisted(target_name, target_whitelist):
            msg = dedent(
                f"""\
